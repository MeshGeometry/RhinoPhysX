...
//#include "PsFile.h"
#include "foundation/../../src/foundation/include/PsBitUtils.h"
#include "foundation/../../src/foundation/include/PsAllocator.h"
...
namespace PhysXEngine
{
		...
		mParticleValidityDevice(0)
	{
		...
		cuMemAlloc(&mParticleValidityDevice, sizeof(PxU32)*(_mParticleSystem->getMaxParticles() + 31) >> 5);
		cuMemAlloc(&mParticleLifetimeDevice, sizeof(PxU32)*(_mParticleSystem->getMaxParticles()));

		if(mUseInstancedMeshes)
		{
			cuMemAlloc(&mParticlesOrientationsDevice, sizeof(PxMat33)*_mParticleSystem->getMaxParticles());
			cuMemcpyHtoDAsync(mParticlesOrientationsDevice, &mParticlesOrientations[0], sizeof(PxMat33)*_mParticleSystem->getMaxParticles(), 0);
		}
#endif
	}
...
#ifdef RENDERER_ENABLE_CUDA_INTEROP
		cuMemFree(mParticleValidityDevice);
		cuMemFree(mParticleLifetimeDevice);
		cuMemFree(mParticlesOrientationsDevice);
#endif
	...
	*/
	...
	{
		float sideNumFloat = pow(3.0f*maxParticles/(4.0f*PxPi), 1.0f/3.0f); 
		PxU32 sideNum = static_cast<PxU32>(ceil(sideNumFloat));
	...
	PxU32 ParticleSystem::createParticleCube(const PxBounds3& aabb, float particleDistance, const PxVec3& vel, PxReal lifetime, PxReal restOffsetVariance)
	{
		...
		aabbDim *= 2.0f;
		unsigned sideNumX = (unsigned)PxMax(1.0f, floor(aabbDim.x / particleDistance));
		unsigned sideNumY = (unsigned)PxMax(1.0f, floor(aabbDim.y / particleDistance));
		unsigned sideNumZ = (unsigned)PxMax(1.0f, floor(aabbDim.z / particleDistance));
